...
pub const LEET_STATS_COMMAND_MD: &str = r#"# /leet-stats — Token savings report with 1337
...
**Without leet** (cost per new session with no compressed memory):
```
tokens_without_leet = total_records × 400
```

**With leet** (cost of a typical `leet_recall` with limit=5):
```
...
**Context already absorbed by the pyramid** (never needs to enter context again):
```
...
Show it in this format (adapt to Portuguese if the user speaks Portuguese):
...
Memory pyramid:
  L0 (raw):  X live  (Y absorbed)
  L1:        X live  (Y absorbed)
  L2+:       X live  (Y absorbed)
  Total:     N records  (V live · C absorbed)

─────────────────────────────────────────────

New-session context cost:
...
Make it clear these are estimates. Don't expose internal COGON numbers or level details to the user — only if they ask.
"#;
...
mod stats_tests {
    ...
    fn stats_command_has_two_sides() {
        assert!(LEET_STATS_COMMAND_MD.contains("Without leet"));
        assert!(LEET_STATS_COMMAND_MD.contains("With leet"));
    }
