...
// ─── /leet-stats slash command ────────────────────────────────────────────────

pub const LEET_STATS_COMMAND_MD: &str = r#"# /leet-stats — Relatório de economia de tokens com 1337
...
Se o projeto não tiver um store ainda, mostre:
"Store vazio — nenhum contexto foi salvo ainda. Use `leet_remember` para começar."
E encerre.

## Passo 2 — Estado atual do recall

Chame `leet_recall` com `limit=50` para obter todos os registros vivos.
...
**Sem leet** (custo por sessão nova sem memória comprimida):
```
tokens_sem_leet = total_records × 400
```

**Com leet** (custo de um `leet_recall` típico com limit=5):
```
...
**Contexto já absorvido pela pirâmide** (nunca mais precisa entrar no contexto):
```
...
Mostre neste formato (adapte para português se o usuário fala português):
...
Pirâmide de memória:
  L0 (raw):  X vivos  (Y absorvidos)
  L1:        X vivos  (Y absorvidos)
  L2+:       X vivos  (Y absorvidos)
  Total:     N registros  (V vivos · C absorvidos)

─────────────────────────────────────────────

Custo de contexto por sessão nova:
...
Deixe claro que são estimativas. Não exponha números internos do COGON nem detalhes de nível ao usuário — só se ele pedir.
"#;
...
mod stats_tests {
    ...
    #[test]
    fn stats_command_has_two_sides() {
        assert!(LEET_STATS_COMMAND_MD.contains("Sem leet"));
        assert!(LEET_STATS_COMMAND_MD.contains("Com leet"));
    }

    #[test]
    fn stats_command_uses_bash_step() {
        assert!(LEET_STATS_COMMAND_MD.contains("consolidate inspect --json"));
    }

    #[test]
    fn stats_command_uses_recall_step() {
        assert!(LEET_STATS_COMMAND_MD.contains("leet_recall"));
    }
}
