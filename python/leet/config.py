"""
Unified configuration system for the 1337 SDK.
...
class ProjectionConfig:
    """Semantic projection configuration."""
    backend: str = "mock"  # 'mock', 'anthropic', 'openai', 'service'

    # API keys (preferably via env vars)
...
class LeetConfig:
    """
    Complete configuration for the 1337 SDK.

    This is the central configuration that spans all modules.
    """
    ...
    def from_dict(cls, data: dict) -> "LeetConfig":
        """Creates a configuration from a dict."""
        # Extract sub-configs
    ...
    def from_toml(cls, path: Union[str, Path]) -> "LeetConfig":
        ...
        try:
            ...
            try:
                ...
                raise ImportError("TOML library not installed. Install with: pip install toml")
    ...
    def load(
        ...
        """
        Loads configuration from a file and/or environment variables.
        ...
        Args:
            path: Path to the configuration file (JSON, YAML, TOML)
        ...
        if use_env:
            ...
            # Simple merge: env always wins
            config = _merge_configs(config, env_config)

        return config

    def to_dict(self) -> dict:
        """Converts configuration to a dict."""
        return {
    ...
    def validate(self) -> list[str]:
        ...
        if self.projection.backend not in ("mock", "anthropic", "openai", "service"):
            errors.append(f"Invalid projection backend: {self.projection.backend}")
...
def _merge_configs(base: LeetConfig, override: LeetConfig) -> LeetConfig:
    ...
    Override takes priority over base for non-default values.
    """
    def is_default(field_value, field_type):
        """Checks whether a value is the dataclass default."""
    ...
    # Simple: returns override if it has non-default values
    ...
    for field_name, field_def in LeetConfig.__dataclass_fields__.items():
        ...
        if hasattr(base_val, '__dataclass_fields__'):
            ...
            # Use override if it's not default
...
def _merge_dataclasses(base, override):
    ...
    for field_name in base.__dataclass_fields__:
        ...
        # Override takes priority if not an "empty" value
