...
const RULES: &[Rule] = &[
    // ── Block S ──────────────────────────────────────────────────────────────
    Rule { keywords: &["essência", "essencia", "identity", "identidade", "núcleo", "nucleo", "core", "essence"], axis: S1_ESSENCIA, value: 0.9 },
    Rule { keywords: &["similar", "análogo", "analogous", "parecido", "like", "resembles", "espelhado"], axis: S2_CORRESPONDENCIA, value: 0.8 },
    Rule { keywords: &["mudança", "mudanca", "change", "transformação", "transformacao", "evolução", "evolucao", "transition", "migração"], axis: S3_VIBRACAO, value: 0.85 },
    Rule { keywords: &["versus", " vs ", "oposto", "opposite", "extremo", "extreme", "espectro", "spectrum", "polaridade"], axis: S4_POLARIDADE, value: 0.75 },
    Rule { keywords: &["ciclo", "cycle", "periódico", "periodico", "periodic", "rotina", "routine", "padrão recorrente", "recurring"], axis: S5_RITMO, value: 0.8 },
    Rule { keywords: &["porque", "causa", "cause", "resultado de", "result of", "consequência", "consequencia", "therefore", "portanto", "logo"], axis: S6_CAUSA_EFEITO, value: 0.85 },
    Rule { keywords: &["gera ", "generates", "ativo ", "active", "iniciativa", "initiative", "proativo", "proactive"], axis: S7_GENERO, value: 0.75 },
    Rule { keywords: &["sistema", "system", "conjunto", "emergente", "emergent", "rede ", "network", "ecosistema", "ecossistema"], axis: S8_SISTEMA, value: 0.85 },

    // ── Block D ──────────────────────────────────────────────────────────────
    Rule { keywords: &["status", "estado", "state", "condição", "condicao", "condition", "health", "saúde", "saude", "situação", "situacao"], axis: D1_ESTADO, value: 0.85 },
    Rule { keywords: &["processo", "process", "deploy", "pipeline", "executando", "running", "rodando", "workflow", "fluxo"], axis: D2_PROCESSO, value: 0.85 },
    Rule { keywords: &["relacionado", "related", "conectado", "connected", "dependency", "dependência", "dependencia", "between", "entre "], axis: D3_RELACAO, value: 0.75 },
    Rule { keywords: &["sinal", "signal", "evento", "event", "alerta ", "alert", "notificação", "notificacao", "notification", "trigger"], axis: D4_SINAL, value: 0.8 },
    Rule { keywords: &["estável", "estavel", "stable", "equilíbrio", "equilibrio", "equilibrium", "instável", "instavel", "unstable", "oscilando"], axis: D5_ESTABILIDADE, value: 0.75 },
    // D6 valence — positive
    Rule { keywords: &["bom", "ótimo", "otimo", "good", "great", "excellent", "perfeito", "perfect", "funciona", "works", " ok ", "okay", "sucesso", "success"], axis: D6_VALENCIA_ONT, value: 0.85 },
    // D6 valence — negative (overrides positive if both match)
    Rule { keywords: &[" ruim", " mau ", "bad ", "horrible", "péssimo", "pessimo", "terrible", "errado", "wrong ", "falhou", "failed"], axis: D6_VALENCIA_ONT, value: 0.15 },
    Rule { keywords: &["origem", "source", "causado por", "caused by", "triggered by", "raiz", "root cause", "motivo", "reason"], axis: D7_CAUSALIDADE, value: 0.8 },
    Rule { keywords: &["confirmado", "confirmed", "verificado", "verified", "testado", "tested", "proven", "comprovado", "checked"], axis: D8_VERIFICABILIDADE, value: 0.9 },

    // ── Block G ──────────────────────────────────────────────────────────────
    Rule { keywords: &["quando", "when", "às ", "hora ", "horário", "horario", "data ", "date ", "schedule", "agendado", "timestamp"], axis: G1_TEMPORALIDADE, value: 0.85 },
    // G2 temporal anchor — past (lowers toward 0)
    Rule { keywords: &["ontem", "yesterday", "anteriormente", "previously", "antes ", "before ", "passado", "último", "ultimo", "last "], axis: G2_ANCORA_TEMPORAL, value: 0.1 },
    // G2 temporal anchor — future (raises toward 1)
    Rule { keywords: &["amanhã", "amanha", "tomorrow", "próximo", "proximo", "next ", "futuro", "future", " vai ", "will ", "deverá", "devera", "planned", "planejado"], axis: G2_ANCORA_TEMPORAL, value: 0.9 },
    // G3 completeness — resolved
    Rule { keywords: &["completo", "complete", "done", "finalizado", "finished", "resolvido", "resolved", "concluído", "concluido", "fechado", "closed"], axis: G3_COMPLETUDE, value: 0.9 },
    // G3 completeness — pending
    Rule { keywords: &["pendente", "pending", "incompleto", "incomplete", "em andamento", "in progress", "wip", "aberto", "open", "bloqueado", "blocked"], axis: G3_COMPLETUDE, value: 0.2 },
    // G4 reversibility — reversible
    Rule { keywords: &["reverter", "revert", "desfazer", "undo", "rollback", "voltar atrás", "restore", "restaurar"], axis: G4_REVERSIBILIDADE, value: 0.9 },
    // G4 reversibility — irreversible
    Rule { keywords: &["irreversível", "irreversivel", "irreversible", "definitivo", "permanent", "permanente", "deletado", "deleted"], axis: G4_REVERSIBILIDADE, value: 0.1 },
    Rule { keywords: &["complexo", "complex", "difícil", "dificil", "difficult", "complicado", "complicated", "pesado", "heavy", "trabalhoso"], axis: G5_CARGA, value: 0.85 },
    // G6 origin — observed
    Rule { keywords: &["observado", "observed", "detectado", "detected", "visto", "seen", "log ", "monitored", "medido", "measured"], axis: G6_ORIGEM, value: 0.9 },
    // G6 origin — inferred
    Rule { keywords: &["inferido", "inferred", "estimado", "estimated", "parece", "seems", "provavelmente", "probably"], axis: G6_ORIGEM, value: 0.5 },
    // G6 origin — assumed
    Rule { keywords: &["assumido", "assumed", "suposição", "suposicao", "assumption", "acredito", "believe"], axis: G6_ORIGEM, value: 0.1 },
    // G7 epistemic valence — confirmatory
    Rule { keywords: &["certamente", "certainly", "definitivamente", "definitely", "confirmado", "confirmed", "correto", "correct", "provado", "proven"], axis: G7_VALENCIA_EPIST, value: 0.9 },
    // G7 epistemic valence — inconclusive
    Rule { keywords: &["talvez", "maybe", "possibly", "possivelmente", "incerto", "uncertain", "dúvida", "duvida", "unclear"], axis: G7_VALENCIA_EPIST, value: 0.4 },
    // G7 epistemic valence — contradictory
    Rule { keywords: &["contraditório", "contraditorio", "contradictory", "conflito", "conflict", "inconsistente", "inconsistent", "diverge"], axis: G7_VALENCIA_EPIST, value: 0.1 },
    // G8 urgency — critical
    Rule { keywords: &["urgente", "urgent", "imediato", "immediate", "asap", "agora!", "now!", "emergência", "emergencia", "emergency"], axis: G8_URGENCIA, value: 0.95 },
    Rule { keywords: &["crítico", "critico", "critical", "severity", "severidade"], axis: G8_URGENCIA, value: 0.9 },
    // G8 urgency — moderate
    Rule { keywords: &["importante", "important", "atenção", "atencao", "attention", "prioridade", "priority"], axis: G8_URGENCIA, value: 0.7 },
    Rule { keywords: &["status", "monitorar", "monitor", "verificar", "check", "health"], axis: G8_URGENCIA, value: 0.55 },

    // ── Block P ──────────────────────────────────────────────────────────────
    Rule { keywords: &["impacto", "impact", "consequência", "consequencia", "consequence", "efeito", "effect", "resultado"], axis: P1_IMPACTO, value: 0.85 },
    Rule { keywords: &["valor", "value", "importa", "matters", "essencial", "essential", "fundamental", "relevante", "relevant"], axis: P2_VALOR, value: 0.85 },
    // P3 anomaly — error/failure
    Rule { keywords: &["erro", "error", "falha", "failure", "crash", "down", "problema", "problem", "bug", "issue", "quebrado", "broken"], axis: P3_ANOMALIA, value: 0.9 },
    Rule { keywords: &["anomalia", "anomaly", "incomum", "unusual", "estranho", "strange", "unexpected", "inesperado"], axis: P3_ANOMALIA, value: 0.75 },
    Rule { keywords: &["crítico", "critico", "critical"], axis: P3_ANOMALIA, value: 0.85 },
    // P4 affect — positive
    Rule { keywords: &["excelente", "excellent", "ótimo", "otimo", "happy", "feliz", "alegre", "joy", "animado"], axis: P4_AFETO, value: 0.9 },
    // P4 affect — negative
    Rule { keywords: &["preocupante", "concerning", "preocupado", "worried", "triste", "sad", "ansioso", "anxious", "frustrado"], axis: P4_AFETO, value: 0.15 },
    Rule { keywords: &["depende", "depends", "necessita", "needs", "precisa", "requires", "requer", "prerequisite", "prerequisito"], axis: P5_DEPENDENCIA, value: 0.85 },
    // P6 temporal vector — future-oriented
    Rule { keywords: &["futuro", "future", "próximo release", "next release", "planejado", "planned", "roadmap", "vai ser", "will be"], axis: P6_VETOR_TEMPORAL, value: 0.9 },
    // P6 temporal vector — past-oriented
    Rule { keywords: &["histórico", "historico", "history", "legado", "legacy", "versão antiga", "old version", "deprecated"], axis: P6_VETOR_TEMPORAL, value: 0.1 },
    // P7 action
    Rule { keywords: &["fazer", "do ", "executar", "execute", "run ", "implementar", "implement", "realizar", "perform", "ação", "acao", "action"], axis: P7_ACAO, value: 0.9 },
    Rule { keywords: &["verificar", "check", "analisar", "analyze", "investigar", "investigate", "examinar", "examine", "revisar", "review"], axis: P7_ACAO, value: 0.7 },
    // P8 action valence — confirmation
    Rule { keywords: &["confirmar", "confirm", "aprovado", "approved", "aceito", "accepted", " sim ", " yes ", "correto", "correct", "ok ", "done"], axis: P8_VALENCIA_ACAO, value: 0.9 },
    // P8 action valence — alert
    Rule { keywords: &["alerta", "alert", "aviso", "warning", "cuidado", "careful", "atenção urgente", "watch out"], axis: P8_VALENCIA_ACAO, value: 0.1 },
];
...
pub fn cogon_to_nl(cogon: &Cogon, lang: &str) -> String {
    ...
    } else if cogon.sem[G4_REVERSIBILIDADE] < 0.2 && cogon.sem[P6_VETOR_TEMPORAL] > 0.7 {
        // TODO(v0.6): revisit semantic intent of this branch under v0.5.1 axes
    ...
    // ── Confidence qualifier via P6_VETOR_TEMPORAL ────────────────────────
    // TODO(07d): review P6 injection once operators migrate to v0.5.1 semantics
    let confidence: f32 = cogon.sem[P6_VETOR_TEMPORAL];
    if confidence < 0.45 {
