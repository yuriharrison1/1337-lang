...
pub fn decode_cogon(data: &[u8]) -> Result<Cogon, crate::error::LeetError> {
    ...
    // unc (32 bytes)
    let mut unc: SemVec = [0.0; 32];
    for i in 0..32 {
        unc[i] = u8_to_float(payload_data[48 + i]);
    }
...
fn estimate_json_size(_cogon: &Cogon) -> usize {
    // Rough estimate: UUID (~36) + sem array (~200) + unc array (~200) + metadata
    36 + 32 * 6 + 32 * 6 + 100
}
...
mod tests {
    ...
    fn test_encode_decode_roundtrip() {
        let original = make_cogon(0.5, 0.1);
        ...
        for i in 0..32 {
            ...
            assert!((original.unc[i] - decoded.unc[i]).abs() < 0.004);
        }
    }
    ...
    fn test_binary_size_constant() {
        let c1 = make_cogon(0.1, 0.1);
        let c2 = make_cogon(0.9, 0.9);
    ...
    fn test_invalid_magic() {
        let mut data = encode_cogon(&make_cogon(0.5, 0.1));
        data[0] = 0x00;
        data[1] = 0x00;
