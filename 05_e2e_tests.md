Crie testes de integração end-to-end para o Projeto 1337.

Os testes validam o fluxo completo: tipos Python → operadores → validação → bridge → serialização → roundtrip.

Estes testes usam o pacote Python `leet` que já foi criado (prompt anterior).
Todos os testes usam MockProjector — ZERO dependência de API key ou rede.

---

# CONTEXTO RÁPIDO

- COGON_ZERO: sem=[1]*32, unc=[0]*32, id="00000000-0000-0000-0000-000000000000", stamp=0
- 32 eixos: A0-A13(ontológico), B1-B8(epistêmico), C1-C10(pragmático)
- Índices chave: [8]=A8_ESTADO, [9]=A9_PROCESSO, [22]=C1_URGÊNCIA, [24]=C3_AÇÃO, [26]=C5_ANOMALIA
- Operadores: BLEND(sem=α·c1+(1-α)·c2, unc=max), DIST(cosseno ponderado), DELTA, FOCUS, ANOMALY_SCORE
- Regras: R2(DELTA↔ref+patch), R4(DAG sem ciclos), R6(human→urgency), R8(BROADCAST→ANOMALY|SYNC)
- Intents: ASSERT, QUERY, DELTA, SYNC, ANOMALY, ACK
- EdgeTypes: CAUSA, CONDICIONA, CONTRADIZ, REFINA, EMERGE
...
class TestDag:
    ...
    def test_dag_cycle_detection(self):
        ...
        a = Cogon(sem=[0.5] * 32, unc=[0.1] * 32)
        b = Cogon(sem=[0.5] * 32, unc=[0.1] * 32)
...
class TestDeltaCompression:
    def test_delta_only_urgency(self):
        ...
        prev = Cogon(sem=sem_before, unc=[0.1] * 32)
        curr = Cogon(sem=sem_after, unc=[0.1] * 32)
    ...
    def test_patch_clamp(self):
        ...
        base = Cogon(sem=[0.9] * 32, unc=[0.1] * 32)
...
class TestBlend:
    def test_midpoint_blend(self):
        ...
        c1 = Cogon(sem=[1.0] * 32, unc=[0.0] * 32)
        c2 = Cogon(sem=[0.0] * 32, unc=[0.0] * 32)
    ...
    def test_conservative_uncertainty(self):
        ...
        c1 = Cogon(sem=[0.5] * 32, unc=[0.1] * 32)
        c2 = Cogon(sem=[0.5] * 32, unc=[0.9] * 32)
        result = blend(c1, c2, 0.5)
        for u in result.unc:
    ...
    def test_two_agents_different_domains(self):
        ...
        tech = Cogon(sem=[0.5] * 32, unc=[0.2] * 32)
        tech.sem[7] = 0.95   # A7_SISTEMA
        tech.unc[7] = 0.05

        empathic = Cogon(sem=[0.5] * 32, unc=[0.2] * 32)
...
class TestValidation:
    def _make_valid_msg(self, intent=Intent.ASSERT):
        ...
        return Msg1337(
            ...
            receiver=Receiver(agent_id="bbbbbbbb-bbbb-bbbb-bbbb-bbbbbbbbbbbb"),
            intent=intent,
            payload=cogon,
            c5=CanonicalSpace(
                zone_fixed=[0.5] * 32,
...
class TestMsgEnvelope:
    def test_full_envelope_roundtrip(self):
        ...
        cogon = Cogon(sem=[0.7] * 32, unc=[0.1] * 32)
        msg = Msg1337(
            sender="aaaaaaaa-aaaa-aaaa-aaaa-aaaaaaaaaaaa",
            receiver=Receiver(agent_id="bbbbbbbb-bbbb-bbbb-bbbb-bbbbbbbbbbbb"),
    ...
    def test_msg_with_dag_payload(self):
        ...
        a = Cogon(sem=[0.8] * 32, unc=[0.1] * 32)
        b = Cogon(sem=[0.3] * 32, unc=[0.2] * 32)
        dag = Dag.from_root(a)
        dag.add_node(b)
        dag.add_edge(Edge(from_id=a.id, to_id=b.id, edge_type="CAUSA", weight=0.9))
    ...
    def test_msg_with_raw_bridge(self):
        ...
        cogon = Cogon(sem=[0.5] * 32, unc=[0.2] * 32).with_raw(raw)
...
class TestOperatorsE2E:
    ...
    def test_dist_zero_for_identical(self):
        ...
        c = Cogon(sem=[0.5] * 32, unc=[0.0] * 32)
        d = dist(c, c)
        assert d < 0.001

    def test_dist_increases_with_difference(self):
        """Distância aumenta conforme COGONs divergem."""
        base = Cogon(sem=[0.5] * 32, unc=[0.0] * 32)
        similar = Cogon(sem=[0.6] * 32, unc=[0.0] * 32)
        different = Cogon(sem=[0.0] * 32, unc=[0.0] * 32)
    ...
    def test_anomaly_score_outlier(self):
        ...
        history = [Cogon(sem=[0.5] * 32, unc=[0.0] * 32) for _ in range(5)]
        # Outlier: tudo em 0.0
        outlier = Cogon(sem=[0.0] * 32, unc=[0.0] * 32)
    ...
    def test_anomaly_score_normal(self):
        ...
        history = [Cogon(sem=[0.5] * 32, unc=[0.0] * 32) for _ in range(5)]
        normal = Cogon(sem=[0.5] * 32, unc=[0.0] * 32)
...
1. `pytest tests/test_e2e.py -v` passa TODOS os testes (25+)
2. ZERO dependência de API key ou rede (tudo MockProjector)
3. Cobre: COGON_ZERO, roundtrip, DAG, DELTA, BLEND, validação R1-R21, MSG_1337, operadores, eixos
4. Cada test class tem docstring explicando o que testa
5. Cada test method tem docstring explicando o cenário
6. Os testes servem como documentação executável da spec

Se algum teste falhar porque a implementação (prompt anterior) tem um bug, CONSERTE o bug na implementação — o teste está correto conforme a spec.
